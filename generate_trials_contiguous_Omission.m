function generate_trials_contiguous_Omission(standardParams, deviantParams1, deviantProbability1, interstimulusInterval, numTrials, paramsDir)
    ...
    % minimumSpacing = 2; %for cross refactoriness minimumSpacing needs to be 2
    minimumSpacing = 9:1:17;
...
%         trialTypes{trialIdx} = 'D';
    ...
    for i=1:1:numDeviants1
      XX{i}=  deviantIndices(i):deviantIndices(i)+ randi([0,4],1);%choose whether to have only 1 omission or up to 5omissions in a row
    end
%     dd=2:2:numDeviants1-2;
%     XX(dd,:)=[];
